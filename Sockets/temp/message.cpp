  ...
  m_transfer_encodings_(__encodings)
{
    ...
    m_header_ = header_t(field_name::TRANSFER_ENCODING, field_value.str());
}
...
message::message(
    message_traits::method __method,
    message_traits::target __target,
    std::string            __data)
{
    message_traits_ptr traits((message_traits*)
        (new request_traits(__method, __target)));
    ...
    std::string*             __fixed)
{
    ...
    if (__dynamic && __dynamic->callback) {
        ...
        if (m_traits_ && m_traits_->permit_length_header()) {
            ...
            m_has_body_  = m_traits_->permit_body();
            header_t header(field_name::CONTENT_LENGTH, os.str());
...
{
    if (__message.type() == message_traits::message_type::UNKNOWN)
...
message::_M_process_header_line(parser_context& context)
{
    ...
    try {
        headers.push_back(header_t(buffer.substr(0, result)));
...
message::_M_process_chunk_size(parser_context& context)
{
    ...
    if (chunk_size == 0) {
        ...
        context.target->m_has_body_ = true;
        context.target->m_headers_[body_info[1]] = header_t(
...
message::_M_process_chunk_trailer(parser_context& context)
{
    ...
    try {
        header_t header(buffer.substr(0, result));
...
message::type() const noexcept
{
    ...
    else return message_traits::message_type::UNKNOWN;
}
...
void
message::headers(std::vector<header_t> __list)
{
    const int k_erase_length_headers = 0;
    const int k_throw_on_duplicate = 1;
    
    std::vector<header_t> next;
