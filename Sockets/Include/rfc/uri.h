...
namespace impact {
	typedef struct uri_parser_opts {
		std::function<bool(const std::string&)> expect_scheme;
		std::function<bool(const std::string&)> expect_userinfo;
		std::function<bool(const std::string&)> expect_host;
	} UriParserOptions;
	...
	protected:
		struct uri_parser_opts m_parser_options_;
	...
	private:
		struct parser_context {
			...
			std::string*           data;
			uri*                   result;
			struct uri_parser_opts parser_opts;
		};
